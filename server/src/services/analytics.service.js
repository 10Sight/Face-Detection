...
export const fetchAdvancedStats = async () => {
    ...
    try {
        ...
        logger.error(`[AnalyticsService] Health fetch failed: ${error.message}`);
        throw error;
    }
};
...
export const searchByEmbedding = async (targetEmbedding, threshold = 0.5, limit = 50) => {
    logger.info(`[AnalyticsService] Initiating high-performance forensic scan across 30d logs`);
    ...
    logger.debug(`[AnalyticsService] Forensic scan matched ${matches.length} frames.`);
    return matches;
};
...
export const fetchReportData = async (startDate, endDate, page = 1, limit = 10) => {
    ...
    try {
        ...
        logger.error(`[AnalyticsService] Report generation failed: ${error.message}`);
        throw error;
    }
};
